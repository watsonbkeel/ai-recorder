...
- 默认数据库是 `ai_recorder`。
- 微信小程序 AppID 是 `wxf73895336690e9a6`。AppID 是项目边界配置，不是密钥；WeChat Secret、AI key、PAT 等真实密钥只能放在被 git 忽略的本地文件。
...
前端视觉必须从 `miniprogram/images/loading-1.jpg` 延展；根目录 `img/` 仅作为被忽略的本地视觉草稿目录。视觉方向包括奶油色背景、柔和粉绿配色、珊瑚粉/暖橙主按钮、薄荷绿辅助色、浅奶油卡片、低饱和轻插画、家庭客厅感、留声机、信封、声波和暖光 AI 助手意象。

- 主背景使用 `#f6efe2` 一类奶油暖底，卡片使用 `#fff8ec` 一类浅奶油色。
- 主按钮使用珊瑚粉/暖橙胶囊按钮，次按钮使用奶油白底和浅棕边框。
- 卡片使用 20-28rpx 圆角、柔和阴影和低对比边框。
- 标签可以使用薄荷绿、浅粉、淡蓝，但必须低饱和。
- AI 区域表现为“暖心表达助手”，强调整理表达、帮助说清楚、让家人更容易理解。
