...
- 默认数据库是 `ai_recorder`。
- 完整微信小程序 AppID 属于本地私有配置，不得写入仓库、文档、日志、提交信息或对外回复；已跟踪的 `miniprogram/project.config.json` 只能使用 `touristappid` 占位。
- WeChat Secret、AI key、PAT、完整 AppID 等敏感配置只能放在被 git 忽略的本地文件或微信开发者工具本地配置中，排查问题时只能描述配置项名称，不能回显真实值。
...
The primary family identity UX is slot selection: top row father/mother, child row `child_1` / `child_2` / `child_3`. Father and mother imply relationship and gender; child slots require only son/daughter selection plus family nickname and optional profile fields.

Message visibility supports `private` (指定家人), `family` (全家可见), and `self` (仅自己). Backend services must enforce these scopes and must not rely on frontend receiver lists for full-family coverage. `private` messages may target registered member ids and/or unoccupied family slot keys so users can leave a message before the intended family member registers. `self` messages are self-notes for整理自己的想法: they create no receivers or notifications, show no reply composer, and backend reply/AI reply endpoints must reject them.
...
前端视觉必须从根目录参考图 `img/loading-1.jpg` 延展；小程序运行时页面引用 `miniprogram/images/loading-1.jpg`。除这张参考图外，根目录 `img/` 下其他视觉草稿默认忽略。视觉方向包括奶油色背景、柔和粉绿配色、珊瑚粉/暖橙主按钮、薄荷绿辅助色、浅奶油卡片、低饱和轻插画、家庭客厅感、留声机、信封、声波和暖光 AI 助手意象。

- 主背景使用 `#f6efe2` 一类奶油暖底，卡片使用 `#fff8ec` 一类浅奶油色。
- 主按钮使用珊瑚粉/暖橙胶囊按钮，次按钮使用奶油白底和浅棕边框。
- 卡片使用 20-28rpx 圆角、柔和阴影和低对比边框。
- 标签可以使用薄荷绿、浅粉、淡蓝，但必须低饱和。
- AI 区域表现为柔和的表达助手，强调整理表达、帮助说清楚、让家人更容易理解；普通发送流程默认使用 AI 整理，不需要额外主按钮。
- 避免黑紫 AI 风、冷蓝后台风、强表格感、强边框、举报/审判/诊断感、社交评论区风格或旧班级日记本风格。
